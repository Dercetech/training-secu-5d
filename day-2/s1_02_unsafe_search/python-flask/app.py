"""Deliberately vulnerable Flask search for the local SQL injection lab."""
...
def build_unsafe_query(email: str) -> str:
...
def create_app(database_path: Path | None = None) -> Flask:
    ...
    def search_users():
        ...
        sql = build_unsafe_query(email)
        started = time.perf_counter()
        try:
            with closing(sqlite3.connect(selected_database)) as connection:
                connection.row_factory = sqlite3.Row
                connection.execute("PRAGMA query_only = ON")
                rows = [dict(row) for row in connection.execute(sql).fetchmany(21)]
        except sqlite3.Error as error:
            return jsonify(
                ok=False,
                received={"method": "GET", "path": "/api/search", "email": email},
                sql=sql,
