"""Flask flavour for the local IDOR and authorization middleware lab."""
    ...
    def require_owner(view):
        ...
        def wrapped(note_id: int):
            ...
            if auth_error:
                return auth_error
            # TODO :
            # 1. charger la note demandée avec document_by_id(note_id) ;
            # 2. renvoyer 404 si elle n’existe pas ;
            # 3. comparer son owner à g.current_user["username"] ;
            # 4. renvoyer 403 si les deux valeurs diffèrent ;
            # 5. appeler view(note_id, note) si l’accès est autorisé.
            return jsonify(
                ok=False,
                error="Middleware require_owner à implémenter.",
            ), 501
